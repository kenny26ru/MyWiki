...
@Inject работает с библиотекой javax.inject.

Размещается над полями, методами и конструкциями.

@Inject как и @Autowired в первую очередь пытается подключить зависимость по типу, по описанию и только потом по имени.

Если имя переменной ссылки на класс отличаются от имени компонента, но они одинакового типа, зависимость все равно будет
подтянута.

Если несколько бинов кандидатов на внедрение, нужно использовать аннотацию @Qualifier.

При использовании имени бина, берем аннотацию @Named
...
Аспектно-ориентированное программирование АОП - парадигма программирования, целью которой является повышение модульности
за счет разделения междисциплинарных задач.

Это достигается путем добавления дополнительного поведения к существующему коду без изменения самого кода.

АОП предоставляет возможность реализации в одном месте сквозной логики. То есть логики, которая применяется к множеству
частей приложения и обеспечения автоматического применения этой логики по всему приложению.

Подход Spring AOP заключается в создании "динамических прокси" для целевых объектов и привязыванию объектов к 
конфигурированному совету для выполнения сквозной логики. 

### 33. PROXY что это, для чего и как применяется в Spring ?

Proxy - паттерн заместитель - объект-заместитель, который управляет доступом к другому объекту. Объект прокси 
перехватывает вызовы к реальному объекту.

В Spring прокси оборачивает bean, который может добавлять функционал до или после вызова основного метода.

Spring AOP использует динамические прокси (на основе интерфейсов), либо CGLIB, на основе класса. 

Если есть выбор, лучше использовать динамические прокси. 

Если целевой объект реализует интерфейс, то будет использоваться dynamic proxy JDK, иначе CGLIB. 
